...
## Verbindungsprüfung über fremde Netze — 20.09.2026

- Nach dem neuen Senderstart ist der Passwortteil laut Nutzer-Ausgabe erhalten. Ein ausschließlich empfangender Test bestätigte nun am echten Sender einen verbundenen Peer, erfolgreich entschlüsselte Verbindungsdaten und laufendes Video mit 1280×576. Der Nutzer meldet weiterhin „Verbinden“ in Safari auf dem iPhone über mobile Daten beziehungsweise ein anderes WLAN.
- **Weiterer Konfigurationsfehler bestätigt:** Der feste TLS-TURN-Eintrag `turn.obs.ninja:443` lieferte keine Relay-Kandidaten (Fehler 701). Erreichbare Alias-Varianten mit den bisherigen Zugangsdaten meldeten Fehler 401. Ein erzwungener Relay-Empfang mit der alten Konfiguration erhielt ein korrekt entschlüsseltes Angebot, konnte aber keine Medienverbindung aufbauen. Die Meldungen zu `unload` und `slider-vertical` erklären diesen Transportfehler nicht.
- Die aktuellen offiziell ausgelieferten Server lieferten innerhalb etwa 1–1,3 Sekunden Relay-Kandidaten über UDP und TLS. Der feste `turn`-Parameter wurde entfernt; Sender und Zuschauer verwenden nun VDOs aktuelle Serverauswahl. Direkte Verbindungen bleiben erlaubt.
- Der geladene Mirror verarbeitet die bisher übergebenen `autorecover`-/`autorelay`-Parameter nicht. Sie wurden entfernt; es wird keine dadurch garantierte Wiederherstellung behauptet. Die externe TURN-Listenabfrage kann weiterhin verzögert sein: ihr interner Timeout führt im Fehlerfall zu einer weiteren Anfrage ohne harte Zeitgrenze. Die eigene Player-Startüberwachung bleibt erhalten.
- **Echter synthetischer Relay-Test bestanden:** getrennte Testidentität, Zuschauer mit erzwungener Relay-Policy, ausgewählte Verbindung tatsächlich vom Typ Relay/UDP. LIVE nach 6025 ms; danach weitere 20 Sekunden mit fortschreitenden Videoframes bis 1317, 1280×720 bei ungefähr 59 FPS. VP8-Videodaten und Opus-Audiodaten samt Audioenergie steigen kontinuierlich. TLS-Kandidaten wurden ebenfalls gefunden, die ausgewählte Medienverbindung verwendete UDP.
- **19 Kernprüfungen, 25 Chrome-Prüfungen und 2 simulierte WebKit-Touch-Prüfungen bestanden.** Keine Kamera, kein Mikrofon und kein echter Desktop wurden für den synthetischen Test aufgenommen. Ein erfolgreicher Empfang und hörbarer Quest-Ton auf dem physischen iPhone sind weiterhin vom Nutzer zu bestätigen.
- **Anschließende Nutzerbestätigung am echten iPhone:** Nach Veröffentlichung der korrigierten TURN-Auswahl und Neustart des Senders meldet der Nutzer für Safari über mobile Daten beziehungsweise ein anderes WLAN: **„Bild und Ton funktionieren“**. Damit ist der zuvor offene Empfangstest auf seinem Gerät bestanden; eine Aussage zur Dauerstabilität oder beliebig vielen Zuschauern folgt daraus nicht.

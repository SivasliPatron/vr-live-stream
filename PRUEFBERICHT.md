...
## Player- und Stillstandsreparatur vom 13. September 2026

Abschlussprüfung: **71/71 Modul-/Serverprüfungen** und **66 Browserprüfungen** bestanden; **3 WebKit-Audiosignaltests** wegen fehlender Web-Audio-Unterstützung ausdrücklich übersprungen. `git diff --check` ist sauber. Die Prüfung des gesamten Git-Verlaufs fand weder Sender-URLs noch den aktuellen Publisher-Token; private Dateien sind nicht versioniert. Der echte Launcher-Prüfmodus bestätigte die unveränderte lokale Einrichtung.

Die folgenden Ergebnisse ergänzen die älteren Prüfstände; deren damalige Aussagen über unveröffentlichte Änderungen und frühere Zeitfenster gelten nur für den jeweiligen Abschnitt.

Nach dem öffentlichen Zuschauertest wurde auch der lokale Sender-URL-Builder auf dieselbe offizielle Ausweichseite, den gleichen Salt und den vorhandenen TLS-Relay-Ausweichserver umgestellt. Tests vergleichen nun diese gemeinsamen Netzwerkeinstellungen beider Rollen. Der laufende Sender wurde nicht geschlossen, keine Freigabe automatisch erteilt und kein Code gedreht. Erst der nächste bewusste Start über die vorhandene 02-Verknüpfung verwendet die neue Senderadresse und erzeugt wie üblich einen neuen Code. Damit bleibt der gemeinsame Live-Test des aktualisierten Senders noch offen.

- Der Zuschauer verwendet die offizielle GitHub-Ausweichseite von VDO.Ninja. Der echte Chrome-Test lud alle 17 Skripte der Seite einschließlich ihrer lokalen Module vollständig und erhielt innerhalb der ersten 10 Sekunden gültige Player-Statistiken, ohne JavaScriptfehler. Vorher brachen Downloads der Hauptseite nach 18–20 Sekunden unvollständig ab. Sender-Salt, Audience-Token, PIN-Fragment und genaue Origin-/Source-Prüfung bleiben erhalten.
- Ein zweiter Startfehler liegt im VDO-Helfer `fetchWithTimeout`: Nach einem abgebrochenen Download der TURN-Liste erfolgt ein erneuter `fetch` ohne Frist; die JSON-Verarbeitung ist ebenfalls nicht vom ursprünglichen Timer umfasst. `sessionconnect` wartet auf diese Auswahl vor dem WebSocket-Start. Der Ablauf wurde in Hauptseite und Mirror gelesen; im echten Browser blieb die WebSocket-Erstellung nach einem abgebrochenen Listen-Download aus.
- Die Viewer-URL konfiguriert deshalb VDOs vorhandenen deutschen TLS-Ausweichserver explizit. Seine Zugangswerte sind öffentliche VDO-Standardwerte, keine privaten Senderdaten. Eine echte Chrome-ICE-Prüfung erhielt nach 620 ms Relay-Kandidaten ohne Fehler. Direkte Verbindungen sind weiterhin erlaubt, der explizite Relay-Modus bleibt erhalten. Einschränkung: keine automatische regionale Relay-Auswahl mehr. Dies ist kein SFU und reduziert nicht den Upload pro Zuschauer.
- Der neue Video-Fortschrittsmonitor bevorzugt dekodierte Frames. Steigende Bytes verdecken keinen Decoderstillstand. Nach 12 Sekunden kontinuierlich bestätigtem Stillstand erfolgt einmal eine Viewer-zu-Sender-Keyframe-Anforderung, nach 30 Sekunden ein geordneter Neustart. Neue Bilder lösen die Warnung ohne Playerwechsel auf. Zählerrücksprünge, Statistiklücken und Hintergrundzeit werden separat behandelt. Erste Bilder im Hintergrund beenden trotzdem die Startfrist.

**Validierungsgrenze:** Die lokalen Unit-/Browserprüfungen simulieren den VDO-Player und beweisen keine flüssige Quest-Übertragung. Der zusätzlich autorisierte echte Test mit dem bestehenden Zuschauer-Code empfing trotz laut Nutzer laufendem Sender kein Livebild. Nach Umgehung der TURN-Listenabfrage begann der Audience-WebSocket früh; er öffnete sich nach einem fehlgeschlagenen Versuch, lieferte aber bis Testende keine Nachrichten und keinen Videostream. Ein separater WebSocket-Upgrade-Test zeigte einen Timeout und einen erfolgreichen HTTP-101-Handshake nach rund 5,9 Sekunden. Diese schwankende externe Signalisierung ist durch die Website-Reparatur nicht als behoben nachgewiesen. Echter Spielton, Bildqualität und Last mit mehr als drei Zuschauern bleiben unbestätigt. Der Sender, sein Code und die Publisher-Tokens wurden nicht verändert.

Quellen: [offizielle Ausweichseite](https://docs.vdo.ninja/help/fail-safes-and-backups), [TURN-Parameter](https://docs.vdo.ninja/advanced-settings/turn-and-stun-parameters/turn), [Mirror-Startlogik](https://steveseguin.github.io/vdo.ninja/main.js?ver=1067), [Mirror-Downloadhelfer](https://steveseguin.github.io/vdo.ninja/lib.js?ver=14171), [aktueller Downloadhelfer](https://vdo.ninja/lib.js?ver=14203).
